...
transactionController.prototype.post = async (req,res) => {
  try{
    ...
    if(data.card_id){
      ...
      const transaction = {status:pagarmeTransaction.status,authorization_code:pagarmeTransaction.authorization_code,
        risk_level:pagarmeTransaction.risk_level,card:card._id,userId:req.usuarioLogado.user._id,acquirer_id:pagarmeTransaction.acquirer_id
      };
      ...
      const cardToHash = {card_number:decryptedData.card_number,card_holder_name:decryptedData.card_holder_name,card_cvv:decryptedData.card_cvv,
        card_expiration_date:decryptedData.card_expiration_date };
      const cardHash = await client.security.encrypt(cardToHash);
      const pagarmeTransaction = await client.transactions.create({
        amount:6000 ,  payment_method:'credit_card',card_hash:cardHash,
        customer: {
          name: data.name,
          external_id: "#3311",
          email: data.email,
      ...
      card_id:cardAux.id,userId:req.usuarioLogado.user._id, brand:cardAux.brand,holder_name:cardAux.holder_name,cardNumber:cardAux.first_digits+"******"+cardAux.last_digits};
